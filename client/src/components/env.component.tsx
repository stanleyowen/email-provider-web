  ...
  return (
    <div className="m-10">
      ...
      <FormControl fullWidth className="mt-10 mb-10">
        ...
        >
          {process.env.REACT_APP_ENV_URL?.split(",").map((appName: string) => (
            ...
            {env && env?.length > 0 ? (
              env
                ...
                .map((song: any, index: number) => {
                  return (
                    <TableRow hover key={index} onClick={() => UpdateEnv(song)}>
                      {columns.map((column) => {
                        return (
      ...
      >
        <DialogTitle className="error">
          {data?.properties?.isUpdate ? "Edit" : "Add"} Environment Variable
        </DialogTitle>
        <DialogContent>
          {Object.keys(columns).map((_, index: number) => {
            ...
            return (
              <TextField
                ...
                disabled={data?.properties?.isUpdate && id === "key"}
                onChange={(e) => {
                  !data?.properties?.isUpdate
                    ? handleData(id, e.target.value)
                    : id !== "key" && handleData(id, e.target.value);
                }}
              />
            );
          })}
        ...
        <DialogActions>
          {data?.properties?.isUpdate ? (
            <Button color="error" onDoubleClick={() => SubmitEnv("delete")}>
              Delete
            </Button>
          ) : null}
          ...
          <Button
            ...
            onClick={() =>
              SubmitEnv(data?.properties?.isUpdate === true ? "update" : "add")
            }
          >
